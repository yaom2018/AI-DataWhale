...
async def run_weather_agent(query: str, streaming: bool = True) -> None:
    ...
    try:
        print("正在初始化DeepSeek-MCP表查询agent...")
        # 创建 MCP 服务器连接实例，但不立即运行
        mysql_server = MCPServerStdio(
            name="mcpmysql",
            params={
                "command": "C:\\Windows\\System32\\cmd.exe",
                "args": ["/c", "D:\\306Datawhale\\MCP-Project\\mcpMysql\\.venv\\Scripts\\python.exe", "D:\\306Datawhale\\MCP-Project\\mcpMysql\\main.py"],
                "env":{}
            },
        ...
        try:
            ...
            tools = await mysql_server.list_tools()
            print("\n可用工具列表: ")
            for tool in tools:
            ...
            mysql_agent = Agent(
                name="表查询助手",
                instructions=(
                    "你是一个专业的查询数据库助手，可以帮助用户数据库中表信息。"
                    "用户可能会询问流程表数据信息、商品介绍等信息，请根据用户的问题选择合适的工具进行查询。"
                    "获取到查询结果后，请将中文翻译为英文，在输出的界面同时保留中文。"
                ),
                # 指定 MCP 服务
...
async def main():
    ...
    这个函数实现了一个交互式循环，让用户输入自然语言查询表相关信息
    """

    print("===== DeepSeek MCP 表查询系统 =====")
    print("请输入自然语言查询，例如：")
    print(" - \"物品调拨的信息是什么\"")
    print(" - \"查询苹果产品有哪些\"")
    ...
    try:
        while True:
            # 获取用户输入
            user_query = input("\n请输入您的表查询(输入'quit'或'退出'结束程序): ").strip()

            # 检查是否退出
            if user_query.lower() in ["quit", "退出"]:
                print("感谢使用DeepSeek MCP表查询系统，再见！")
                break
